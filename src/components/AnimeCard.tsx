...
import { getVerifiedAnimeImage, getDynamicAnimeFallbackPoster } from '../data/verifiedImages';
...
export function AnimeCard({ anime, className }: AnimeCardProps) {
  ...
  const getBestPoster = () => {
    // 1. If anime already has its own valid poster, use it!
    if (anime.poster && !isInvalidImage(anime.poster)) {
      return anime.poster;
    }
    ...
    const verified = getVerifiedAnimeImage(anime.title);
    if (verified?.poster && !isInvalidImage(verified.poster)) {
      return verified.poster;
    }
    return getDynamicAnimeFallbackPoster(anime.title);
  };

  const [posterUrl, setPosterUrl] = useState<string>(getBestPoster);
    ...
    if (resolved && !isInvalidImage(resolved)) {
      ...
      fetchAnimeMetadata(anime.title || '').then(meta => {
        if (meta?.poster && !isInvalidImage(meta.poster)) {
          ...
          setPosterUrl(getDynamicAnimeFallbackPoster(anime.title));
        }
      }).catch(() => {
        setPosterUrl(getDynamicAnimeFallbackPoster(anime.title));
      });
  ...
  const handleImageError = () => {
    ...
    // 1. Try verified catalog lookup by exact title
    const verified = getVerifiedAnimeImage(anime.title);
    if (verified?.poster && verified.poster !== posterUrl && !isInvalidImage(verified.poster)) {
      setPosterUrl(verified.poster);
      return;
    }

    // 2. Try catalog match by strict exact title
