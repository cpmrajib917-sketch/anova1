...
export const getOfficialAnovaEmbedUrl = (params: {
  ...
  let url = '';
  const aniOrTarget = cleanAni || targetId;
  const malOrTarget = cleanMal || targetId;

  if (serverVal === 'abyss' || serverVal === 'hd-3') {
    url = `https://embed.su/embed/anime/${aniOrTarget}/${episodeVal}`;
  } else if (serverVal === 'vidstream' || serverVal === 'hd-2') {
    url = `https://vidsrc.pm/embed/anime/${malOrTarget}/${episodeVal}`;
  } else if (serverVal === 'filemoon' || serverVal === 'hd-4') {
    url = `https://vidsrc.to/embed/anime/${aniOrTarget}/${episodeVal}`;
  } else if (serverVal === 'streamtape' || serverVal === 'hd-5') {
    url = `https://vidsrc.cc/v2/embed/anime/${aniOrTarget}/${episodeVal}`;
  } else if (serverVal === 'autoembed') {
    url = `https://autoembed.co/anime/${aniOrTarget}/${episodeVal}`;
  } else if (serverVal === 'vidlink') {
    url = `https://vidlink.pro/anime/${aniOrTarget}/${episodeVal}`;
  } else if (idTypeVal === 'mal') {
    url = `https://vidsrc.pm/embed/anime/${malOrTarget}/${episodeVal}`;
  } else {
    url = `https://vidsrc.net/embed/anime/${aniOrTarget}/${episodeVal}`;
  }
...
export function Watch() {
  ...
  useEffect(() => {
    ...
    setEpisode(prev => (prev !== epVal ? epVal : prev));
  }, [searchParams]);
  ...
  }, [episode, searchParams, setSearchParams]);
...
function formatLanguageOrServerName(sUrl: string, rawName?: string, rawLang?: string): { language: string; provider: string; label: string } {
  ...
  const resolveAnimeIdentifiers = async (animeId: string) => {
    ...
    try {
      ...
      // Direct AniList GraphQL fallback if mapping is still missing numeric IDs
      if ((!anilistId || !malId) && !isNumeric) {
        try {
          const cleanQuery = anime?.title || animeId.replace(/[-_]/g, ' ').replace(/\s+/g, ' ').trim();
          const gqlQuery = `query ($search: String) { Page(page: 1, perPage: 1) { media(search: $search, type: ANIME, sort: POPULARITY_DESC) { id idMal } } }`;
          ...
          if (aniRes.ok) {
            ...
            if (media && media.id) {
              if (!anilistId) anilistId = String(media.id);
              if (!malId) malId = String(media.idMal || media.id);
              if (!animoId || animoId === animeId) animoId = String(media.id);
            }
          }
        } catch (_) {}
      }
      ...
      // Direct AniList GraphQL fallback in catch block
      if ((!anilistId || !malId) && !isNumeric) {
        try {
          const cleanQuery = anime?.title || animeId.replace(/[-_]/g, ' ').replace(/\s+/g, ' ').trim();
          const gqlQuery = `query ($search: String) { Page(page: 1, perPage: 1) { media(search: $search, type: ANIME, sort: POPULARITY_DESC) { id idMal } } }`;
  ...
  useEffect(() => {
    const handlePlayerMessage = (e: MessageEvent) => {
      try {
        ...
        if (isErrorEvent && autoServerSwitch && !isCustomEpisodeRef.current) {
          ...
          isYoutubeVideo ? (
              ...
              {(isCheckingServers || isIframeLoading) ? (
                ...
                <>
                  ...
                  <div className="flex flex-wrap items-center justify-center gap-3">
                    ...
                    <button
                      onClick={() => {
                        const fallbackIds = getBestAvailableIdsSync(id || '', resolvedIds);
                        const sUrl = getPlaybackUrlSync(server, currentIdType, fallbackIds);
                        if (sUrl) {
                          setVerifiedPlaybackUrl(sUrl);
                        }
                        runPlaybackPipeline(server, currentIdType, 0);
                      }}
                      className="px-4 py-2 bg-gradient-to-r from-[#00e5ff] to-cyan-500 hover:brightness-110 text-black text-xs font-bold rounded-xl shadow-lg shadow-cyan-500/20 transition-all flex items-center gap-2"
                    >
                      <Play className="w-3.5 h-3.5 fill-current" />
                      Stream via HD Server
                    </button>
                    {Number(episode) > 1 && (
                      <button
            ...
            <div className="w-full h-full flex flex-col items-center justify-center bg-[#010307] z-20 gap-4 select-none px-4 text-center">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-[#00e5ff] to-cyan-500 hover:brightness-110 text-black text-xs font-bold rounded-xl shadow-lg shadow-cyan-500/20 transition-all flex items-center gap-2 mt-2"
              >
                <Play className="w-3.5 h-3.5 fill-current" />
                Stream via HD Server
              </button>
