...
export function fuzzySearchAnime(
      ...
      if (!MOVIE_TERMS.includes(gClean) && !TV_TERMS.includes(gClean) && gClean !== 'anime') {
        ...
        if (!matched && item.genres) {
          ...
          if (Array.isArray(item.genres)) {
            genreList = item.genres.map(g => (typeof g === 'string' ? g : (g as any)?.name || '').toLowerCase().trim());
          ...
          if (genreList.some(g => {
            ...
            return normG === normGClean || g === gClean || (normG.length >= 3 && normGClean.length >= 3 && (normG.includes(normGClean) || normGClean.includes(normG)));
          })) {
            matched = true;
          }
        }
        ...
        if (!matched) {
          if (normGClean === 'cartoon' || normGClean === 'cartoons') {
            const titleLow = String(item.title || '').toLowerCase();
            const idLow = String(item.id || '').toLowerCase();
            const cartoonKeywords = [
              'ben 10', 'transformers', 'loud house', 'doraemon', 'shin-chan', 'avatar',
              'teen titans', 'rick and morty', 'gravity falls', 'adventure time',
              'tom and jerry', 'spider-man', 'batman', 'oggy'
            ];
            const isCartoon = (item as any).isCartoon === true || 
              String((item as any).type || '').toLowerCase() === 'cartoon' ||
              cartoonKeywords.some(k => titleLow.includes(k) || idLow.includes(k));
            if (isCartoon) matched = true;
          } else if (normGClean === 'hindidubbed' || normGClean === 'hindi' || normGClean === 'hindidub') {
            if (item.hindiAvailable === true || String(item.language || '').toLowerCase().includes('hindi') || (item as any).dubAvailable === true) {
              matched = true;
            }
          ...
          } else if (normGClean === 'toprated') {
            const score = parseFloat(String((item as any).score || (item as any).rating || '0')) || 0;
            if (score >= 8.0) matched = true;
          } else if (normGClean === 'trending' || normGClean === 'popular') {
            const isTrendingOrPopular = Boolean(
              item.categories?.trending || 
              item.categories?.popular || 
              (parseFloat(String((item as any).score || (item as any).rating || '0')) || 0) >= 8.0
            );
            if (isTrendingOrPopular) matched = true;
  ...
  // If query is empty or 'all' or category browsing, return all filtered items ordered by popularity/rating
  const isCategoryBrowsing = Boolean(
    optGenre && (!normQuery || normQuery === optGenre || normalizeString(optGenre) === normQuery)
  );

  const isAllQuery = !normQuery || isCategoryBrowsing || [
    'all',
    'allanime',
    'all anime',
    'all-anime',
  ...
  if (isAllQuery) {
    const filtered = animeList.filter(passesFilters);
    return filtered.sort((a, b) => {
      const scoreA = parseFloat(String((a as any).score || (a as any).rating || '0')) || 0;
      const scoreB = parseFloat(String((b as any).score || (b as any).rating || '0')) || 0;
